...
function Logo({ dark = false, size = 20 }: { dark?: boolean; size?: number }) {
  const c = dark ? "#4F6D7A" : "#4F6D7A";
  const o = dark ? [0.1, 0.2, 0.35] : [0.15, 0.3, 0.5];
  return (
    <svg width={size} height={size} viewBox="0 0 40 40" fill="none">
      <rect x="4" y="4" width="14" height="14" fill={dark ? C.w1 : c} opacity={o[0]} />
      <rect x="12" y="12" width="14" height="14" fill={dark ? C.w1 : c} opacity={o[1]} />
      <rect x="22" y="22" width="14" height="14" fill={dark ? C.w1 : c} opacity={o[2]} />
      <circle cx="11" cy="11" r="2" fill={c} /><circle cx="20" cy="20" r="2" fill={c} /><circle cx="29" cy="29" r="2" fill={c} />
      <line x1="11" y1="11" x2="20" y2="20" stroke={c} strokeWidth="0.75" />
      <line x1="20" y1="20" x2="29" y2="29" stroke={c} strokeWidth="0.75" />
    </svg>
  );
}
...
export default function Home() {
  ...
  const panel: React.CSSProperties = { background: C.w, position: "relative" as const, zIndex: 2 };
  const wallGap = (fragments: { text: string; left: string; top: string; rot: string; color?: string }[]) => (
    <div style={{ background: C.wall, height: 48, position: "relative", overflow: "hidden" }}>
      {fragments.map((f, i) => (
        <span key={i} className="cm-df" style={m({ left: f.left, top: f.top, fontSize: 8, color: f.color || C.w4, opacity: f.color ? 0.06 : 0.1, transform: `rotate(${f.rot})` })}>{f.text}</span>
      ))}
    </div>
  );

  return (
    <div ref={r} style={{ fontFamily: F.b }}>
      ...
      <div style={{ background: C.dk, position: "relative", overflow: "hidden" }}>
        ...
        <svg style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.03, pointerEvents: "none" }}>
          <defs><pattern id="dg" x="0" y="0" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M40 0L0 0 0 40" fill="none" stroke={C.s} strokeWidth="0.5" />
          </pattern></defs>
          <rect width="100%" height="100%" fill="url(#dg)" />
        </svg>
        {/* Contours */}
        <svg className="cm-topo" style={{ position: "absolute", inset: 0, width: "110%", left: "-5%", height: "100%", pointerEvents: "none", opacity: 0.05 }} viewBox="0 0 1600 400" preserveAspectRatio="none">
          {[100, 180, 260, 340].map((y, i) => (
            <path key={i} d={`M-100 ${y}Q400 ${y - 25} 800 ${y}T1700 ${y}`} fill="none" stroke={C.s} strokeWidth={i === 1 ? 1.2 : 0.6} />
          ))}
        </svg>
        ...
        <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 32px", borderBottom: `0.5px solid rgba(250,250,248,.06)`, position: "sticky", top: 0, zIndex: 50, background: "rgba(20,19,18,.85)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)" }}>
          <a href="/" style={{ display: "flex", alignItems: "center", gap: 8, textDecoration: "none" }}>
            <Logo dark /><span style={{ fontFamily: F.d, fontWeight: 700, fontSize: 15, color: C.w1 }}>ClareMesh</span>
          </a>
          <div style={{ display: "flex", alignItems: "center", gap: 24, fontSize: 13 }}>
            {["Schema", "Docs", "Pricing", "Security"].map(l => (
              <a key={l} href={`/${l.toLowerCase()}`} className="cm-nl cm-nl-d" style={{ color: C.w6, textDecoration: "none" }}>{l}</a>
            ))}
        ...
        <div style={{ position: "relative", zIndex: 2, padding: "72px 32px 64px", maxWidth: 580 }}>
          ...
          <h1 className="cm-a3" style={{ fontFamily: F.d, fontWeight: 700, fontSize: "clamp(44px,5.5vw,64px)", letterSpacing: -2, lineHeight: 1.02, marginBottom: 18, color: C.w }}>
            Clarity through<br />connection
          </h1>
          <p className="cm-a4" style={{ fontSize: 15, lineHeight: 1.7, color: C.w4, maxWidth: 400, marginBottom: 28 }}>
          ...
          <div className="cm-a5" style={{ background: C.dk2, border: bdd, padding: "14px 16px", marginBottom: 24 }}>
            ...
            <div style={m({ fontSize: 12, color: C.w1 })}>
              <span style={{ color: C.s }}>$</span> npm install @claremesh/schema @claremesh/transforms
              <span className="cm-blink" style={{ display: "inline-block", width: 7, height: 13, background: C.s, marginLeft: 3, verticalAlign: "text-bottom" }} />
            </div>
          </div>
      ...
      {/* Threshold gradient */}
      <div style={{ height: 3, background: `linear-gradient(to bottom,${C.dk},${C.wall})` }} />

      {/* ═══ DATA WALL + CONTENT PANELS ═══ */}

      {wallGap([
        { text: "acc_8f2a91c4 | asset | $43,200", left: "8%", top: "16px", rot: "-1deg" },
        { text: "sync_ok: true | 12ms", left: "65%", top: "20px", rot: "0.5deg" },
      ])}
      ...
      <div style={panel}>
        ...
        <section className="cm-rv cm-sg" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr" }}>
          {[
            ...
            <div key={i} className="cm-cell" style={{ padding: 32, borderRight: (i + 1) % 3 !== 0 ? bd : "none", borderBottom: i < 3 ? bd : "none" }}>
      ...
      {wallGap([
        { text: "entity_id: ent_acme_01 | corporation | US", left: "15%", top: "18px", rot: "-1.5deg" },
        { text: "transform: plaid → claremesh", left: "68%", top: "14px", rot: "0.8deg", color: C.s },
      ])}
          ...
          <div style={{ maxWidth: 720, margin: "0 auto" }}>
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 8, marginBottom: 28, flexWrap: "wrap" }}>
              {["Plaid", "Stripe", "QuickBooks", "Xero", "NetSuite", "CSV"].map(s => (
                <span key={s} style={m({ fontSize: 11, padding: "8px 16px", border: bd, color: C.w6, background: C.w5 })}>{s}</span>
              ))}
            </div>
            <div style={{ textAlign: "center", color: C.w2, fontSize: 18, margin: "4px 0" }}>&#8595;</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 6, margin: "8px 0" }}>
              {[
                { n: "Transform", p: "@claremesh/transforms", c: "#0F6E56", bg: "#E1F5EE" },
                { n: "Schema", p: "@claremesh/schema", c: "#534AB7", bg: "#EEEDFE" },
                { n: "Sync", p: "@claremesh/sync", c: "#993C1D", bg: "#FAECE7" },
              ].map(l => (
      ...
      {wallGap([
        { text: "balance: { available: 124200, currency: \"USD\" }", left: "20%", top: "16px", rot: "-1deg" },
      ])}
      ...
      <div style={panel}>
        <section className="cm-rv">
          <div style={{ padding: "56px 32px 16px", textAlign: "center", borderTop: bd }}>
            ...
            <h2 style={{ fontFamily: F.d, fontWeight: 700, fontSize: 28, textAlign: "center", letterSpacing: -0.5, marginBottom: 12 }}>One pipe. Three products.</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", borderTop: bd, borderBottom: bd }}>
            {[
              { n: "Vaultline", ro: "Treasury", rd: "Cash, balances, projections", wr: "Alert rules, scenarios", bc: "#00A3B4" },
              { n: "Castford", ro: "FP&A", rd: "P&L, variance, budgets", wr: "Forecasts, board packages", bc: "#3B6FA0" },
              { n: "Ashford Ledger", ro: "Month-end close", rd: "GL, transactions, entities", wr: "Reconciliation, journal entries", bc: "#7B5EA7" },
            ].map((p, i) => (
      ...
      {wallGap([
        { text: "consent: granted | purpose: financial_sync", left: "12%", top: "18px", rot: "-2deg" },
        { text: "schema v2.4.1 | validated", left: "72%", top: "15px", rot: "1deg", color: C.s },
      ])}
      ...
      <div style={panel}>
        <section className="cm-rv" style={{ borderTop: bd, borderBottom: bd, padding: "64px 32px" }}>
          <div style={{ maxWidth: 720, margin: "0 auto" }}>
            ...
            <h2 style={{ fontFamily: F.d, fontWeight: 700, fontSize: 28, textAlign: "center", letterSpacing: -0.5, marginBottom: 36 }}>Normalized data in four lines</h2>
      ...
      {wallGap([
        { text: "txn_b4c1 | debit | $1,840.00 | reconciled", left: "18%", top: "17px", rot: "-0.5deg" },
      ])}
      ...
      <div style={panel}>
        <section className="cm-rv">
          <div style={{ padding: "56px 32px 12px", textAlign: "center", borderTop: bd }}>
            ...
            <h2 style={{ fontFamily: F.d, fontWeight: 700, fontSize: 28, textAlign: "center", letterSpacing: -0.5, marginBottom: 36 }}>Simple, usage-based pricing</h2>
          </div>
              ...
              <div key={i} className="cm-pc" style={{ padding: 32, display: "flex", flexDirection: "column", borderRight: i < 3 ? bd : "none", ...(t.po ? { borderLeft: `2px solid ${C.s2}`, borderRight: `2px solid ${C.s2}`, margin: "0 -1px", position: "relative" as const, zIndex: 1 } : {}) }}>
                ...
                <a href={t.t === "ENTERPRISE" ? "/contact" : "/signup"} className={t.pr ? "cm-bp" : "cm-bg"}
                  style={{ display: "block", textAlign: "center", padding: 14, marginTop: 32, fontSize: 13, fontWeight: 500, textDecoration: "none",
                    background: t.pr ? C.s2 : C.w5, color: t.pr ? "#fff" : C.w8, border: t.pr ? "none" : bd,
                    ...(t.pr ? { boxShadow: "0 1px 3px rgba(0,0,0,.1)" } : {}) }}>{t.ct}</a>
              </div>
            ))}
          </div>
        </section>
      ...
      {/* Threshold gradient (light→dark) */}
      <div style={{ height: 3, background: `linear-gradient(to bottom,${C.wall},${C.dk})` }} />

      {/* ═══ DARK COMPLIANCE SECTION ═══ */}
      <div style={{ background: C.dk, padding: "64px 32px", color: C.w1 }}>
        <div className="cm-rv" style={{ maxWidth: 600, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: F.d, fontWeight: 700, fontSize: 28, letterSpacing: -0.5, marginBottom: 14, color: C.w }}>22 controls. Zero egress.</h2>
          <p style={{ fontSize: 14, color: C.w6, lineHeight: 1.7, marginBottom: 32, maxWidth: 440, display: "inline-block" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr 1fr", gap: 0, marginBottom: 28 }}>
            {[
              { f: "SOC 2", ct: "8", c: "#4F6D7A" }, { f: "GDPR", ct: "6", c: "#0F6E56" },
              { f: "CCPA", ct: "4", c: "#854F0B" }, { f: "PCI", ct: "2", c: "#534AB7" },
              { f: "SOX", ct: "2", c: "#993C1D" },
            ].map((fw, i) => (
      ...
      {/* ═══ DARK FOOTER ═══ */}
      <footer style={{ background: C.dk, borderTop: bdd }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 32px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <Logo dark size={14} /><span style={m({ fontSize: 10, color: C.w8 })}>CLAREMESH &copy; 2026 FINANCIAL HOLDING LLC</span>
          </div>
