...
export default function SignupPage() {
  ...
  const planFromUrl = searchParams.get("plan") || "open";
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    const { error: signUpError } = await supabase.auth.signUp({
      ...
      options: {
        ...
        data: { org_name: orgName || email.split("@")[1], plan: planFromUrl },
      },
    });
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", minHeight: "100vh", fontFamily: F.b }}>
      ...
      <div style={{ background: "var(--cm-hero)", padding: "48px 40px", display: "flex", flexDirection: "column", justifyContent: "space-between", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <p style={{ fontFamily: F.d, fontWeight: 700, fontSize: "clamp(32px,4vw,44px)", letterSpacing: -2, lineHeight: 1.05, color: "var(--cm-text-hero-h)", marginBottom: 16 }}>
            Normalize your<br />financial data
          </p>
          <p style={{ fontSize: 15, color: "var(--cm-text-hero-b)", lineHeight: 1.7, maxWidth: 320 }}>
            Open-source schema. Bi-directional sync. Your infrastructure. Zero data egress.
          </p>
        </div>

        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={{ display: "flex", gap: 24, marginBottom: 16 }}>
            {[{ n: "22", l: "compliance controls" }, { n: "5", l: "frameworks" }, { n: "0", l: "data egress" }].map((s) => (
              <div key={s.l}>
                <span style={m({ fontSize: 24, fontWeight: 500, color: "var(--cm-text-hero-h)" })}>{s.n}</span>
                <br /><span style={m({ fontSize: 9, color: "var(--cm-text-mono)" })}>{s.l}</span>
              </div>
            ))}
          </div>
          <p style={m({ fontSize: 10, color: "var(--cm-footer-c)" })}>claremesh.com</p>
        </div>
      </div>
      ...
      <div style={{ background: "var(--cm-panel)", padding: "48px 40px", display: "flex", flexDirection: "column", justifyContent: "center", maxWidth: 480, margin: "0 auto", width: "100%" }}>
        <h1 style={{ fontFamily: F.d, fontWeight: 700, fontSize: 26, letterSpacing: -0.5, marginBottom: 6, color: "var(--cm-text-panel-h)" }}>Create your account</h1>
        <p style={{ fontSize: 14, color: "var(--cm-text-panel-b)", marginBottom: 28 }}>
          Start with the Open tier. Upgrade anytime.
        </p>

        {error && (
          <div style={{ padding: "10px 14px", marginBottom: 16, background: "var(--cm-panel-inset)", border: "0.5px solid var(--cm-border-light)", fontSize: 13, color: "#A32D2D" }}>
            {error} {error.includes("already exists") && <a href="/login" style={{ color: "var(--cm-slate)", fontWeight: 500 }}>Sign in instead</a>}
          </div>
        )}

        <form onSubmit={handleSignup}>
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 12, color: "var(--cm-text-panel-b)", display: "block", marginBottom: 5 }}>Work email</label>
          ...
          <div style={{ marginBottom: 14 }}>
            ...
            <input type="password" required minLength={8} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Min 8 characters" style={input} />
          </div>
          <div style={{ marginBottom: 24 }}>
            <label style={{ fontSize: 12, color: "var(--cm-text-panel-b)", display: "block", marginBottom: 5 }}>Organization name</label>
            <input type="text" value={orgName} onChange={(e) => setOrgName(e.target.value)} placeholder="Your company name" style={input} />
          </div>
        ...
        <p style={{ fontSize: 12, color: "var(--cm-text-dim)", textAlign: "center" }}>
          Already have an account? <a href="/login" style={{ color: "var(--cm-slate)", fontWeight: 500, textDecoration: "none" }}>Sign in</a>
        </p>
      </div>
    </div>
  );
